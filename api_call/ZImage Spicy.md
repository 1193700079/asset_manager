...
```plaintext
https://smartstudio-intl.aliyuncs.com/api/v2/services/aigc/ecosystem/zimage-spicy/text-to-image
```
...
| input.workflow_type | string | 是  | 工作流类型，固定值：`zimage_spicy` |
| input.payload | object | 是  | 任务参数对象 |
| input.payload.prompt | string | 是  | 文本提示词，用于指导生成效果 |
| input.payload.width | integer | 否  | 输出图片宽度，范围：256-1536，默认 1024 |
| input.payload.height | integer | 否  | 输出图片高度，范围：256-1536，默认 1536 |
...
```bash
curl --location --request POST 'https://smartstudio-intl.aliyuncs.com/api/v2/services/aigc/ecosystem/zimage-spicy/text-to-image' \
