...
# QwenImageEdit Spicy API 文档

本文档提供 图像编辑 API 的接口说明。
...
| --- | --- |
| provider | ecosystem |
| model | qwenimageedit-spicy |
| capability | image-to-image |

**完整示例 URL**:

```plaintext
https://smartstudio-intl.aliyuncs.com/api/v2/services/aigc/ecosystem/qwenimageedit-spicy/image-to-image
```
...
| input.workflow_type | string | 是 | 工作流类型，固定值：`qwenimageedit_spicy` |
| input.payload | object | 是 | 任务参数对象 |
| input.payload.image | string | 是 | 输入图片的 URL 地址（支持 HTTPS）或 Base64 编码 |
| input.payload.prompt | string | 是 | 文本提示词，用于指导图像编辑效果 |
...
{
  "input": {
    "workflow_type": "qwenimageedit_spicy",
    "payload": {
...
```bash
curl --location --request POST 'https://smartstudio-intl.aliyuncs.com/api/v2/services/aigc/ecosystem/qwenimageedit-spicy/image-to-image' \
